...
1.  What is `a`?
```
a is (Data Type is number)
```
2.  What is `b`?
```
b is (Data Type is string)
```
3.  What is `c`?
```
c is ( Boolean data type)
```
...
```js
var favoriteMovies =["Up","Beauty and the Beast" , "Wall-E","The Lion King", "The Matrix"];
```
